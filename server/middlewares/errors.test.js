...
describe("Given an notFoundErrorHandler middleware,", () => {
  describe("When it gets a request", () => {
    test("Then it should send a response with a 'Endopoint not found (404) （┬┬＿┬┬）' error and a status code of 404", () => {
      const res = mockResponse();
      const expectedError = { error: "Endopoint not found (404) （┬┬＿┬┬）" };
      const req = {};
...
describe("Given an generalErrorHandler middleware,", () => {
  describe("When it gets a request and an error and no error code", () => {
    test("Then it should send a response with a 'All broken :(' error and a status code of 500", () => {
      const res = mockResponse();
      const error = { error: "General error ಥ,_｣ಥ" };
      const req = {};
      const next = () => {};
